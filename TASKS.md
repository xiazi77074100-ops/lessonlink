...
- [x] 家庭単位（兄弟姉妹1人以上）・保護者利用上限・期限付き招待
  - `invitation_children` 中間テーブル追加（`d620651f11a1`）。`POST /invitations` に `child_ids` を追加し、
    1人以上指定すると家庭単位の招待になる（`max_uses` 未指定時はデフォルト2人＝保護者利用上限として扱う）。
  - 招待の利用回数は既存実装のとおり一意な保護者単位でカウント（ログイン試行では増えない）。API/mock LINEで
    guardian 2人まで成功・3人目は`INVITATION_LIMIT_REACHED`になることを確認済み。
  - Admin: 招待発行ダイアログに「家庭を招待」（子供複数選択）/「組織全体で共有」の切り替えを追加、一覧に対象の子供を表示。
  - Parent: 招待に紐づく子供だけを表示し、1回のオンボーディングで複数の兄弟姉妹を順に確認・紐付け（`確認済みN人`→完了）。
    組織招待（子供未指定）は従来どおり名簿から選ぶ形式のまま、同様に複数人ぶん確認できるよう修正。
- [x] Parent: 組織確認 → 子供確認 → 完了の段階表示
- [ ] 375px幅で招待・紐付けフローを確認（実装はモバイル優先CSSのまま拡張。実ブラウザでの目視確認は未実施）
...
1. Phase 13: 375px幅で招待・紐付けフロー（家庭招待・組織招待の両方）を実ブラウザ/実機で目視確認
